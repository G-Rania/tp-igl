...
import { getNotApproved, removeArticle } from '../../api/moderator/articles_api';
import Mod from "../../models/moderator"


export default function RemoveArticle({ isOpen, onClose , id , onRemove}) {
  const [message, setMessage] = useState(''); /*For the message response of the request to the server */
  const [isSuccess, setIsSuccess] = useState(false);
  
  /** for the color of the message */
 ...
 return (
     <div className={`fixed z-50 top-10 w-full h-full flex items-center justify-center ${isOpen ? 'flex' : 'hidden'}`}>
