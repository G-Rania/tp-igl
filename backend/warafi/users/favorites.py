...
def check_favorite(user_id, article_id):
    try:
        favorite = Favorite.objects.get(user_id = user_id, article_id = article_id)
        return True
    except:
        return False
...
@csrf_protect
@csrf_exempt
def get_favorites_elastic(request):
    if request.method == 'POST':
        token_status = auth.check_token(request=request).status_code
        if (token_status == 200):
            data = json.loads(request.body)
