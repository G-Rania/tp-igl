...
def get_mods(request):
    ...
    else: return JsonResponse({"error":"Invalid request method"}, status = 400)
    ...
    if request.method == 'POST':
        ...
        if (token_status == 200):
            ...
            try:
                ...
                return JsonResponse({"message":"Mod created" , "mod_id":mod.id} ,status = 200)
    ...
    if request.method == 'POST':
        ...
        if (token_status == 200):
            try:
                ...
                return JsonResponse({'error': 'infos already exist in another mod'}, status=406)    
        else: return JsonResponse({'error': 'Admin not authenticated'}, status=405) 
